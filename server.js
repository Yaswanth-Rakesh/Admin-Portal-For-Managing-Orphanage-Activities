...
const registerUser = async (req, res, role) => {
  ...
  console.log(`Registration attempt for ${role}:`, req.body);

  if (!email || !password) {
    console.error(`Registration failed: Missing email or password for ${role}`);
    return res.status(400).json({ message: "Email and password are required" });
  }

  if (role === "staff" && (!name || !phone || !email || !jobTitle)) {
    console.error(`Registration failed: Missing required fields for staff: ${JSON.stringify(req.body)}`);
    return res.status(400).json({ message: "Name, phone, email, and job title are required for staff" });
  }

  if (role === "donor" && (!name || !phone || !email)) {
    console.error(`Registration failed: Missing required fields for donor: ${JSON.stringify(req.body)}`);
    return res.status(400).json({ message: "Name, phone, and email are required for donor" });
  }
  ...
  try {
    ...
    db.query(query, [name, email, hashedPassword, phone, address || null, role, role === "staff" ? jobTitle : null], (err, result) => {
      if (err) {
        console.error(`Database error during ${role} registration:`, err);
    ...
    console.error(`Server error during ${role} registration:`, err);
...
app.put("/api/staff/me", verifyToken, restrictTo("staff", "admin"), (req, res) => {
  ...
  if (!name || !email || !phone || !jobTitle) {
    console.error("Missing required fields in /api/staff/me PUT:", { name, email, phone, jobTitle });
  ...
  db.query(query, [name, email, phone, jobTitle, req.user.id], (err, result) => {
    if (err) {
      if (err.code === "ER_DUP_ENTRY") {
        console.error(`Profile update failed: Email ${email} already exists`);
        return res.status(400).json({ message: "Email already exists" });
...
app.get("/api/dashboard/counts", verifyToken, restrictTo("admin"), (req, res) => {
  const queries = {
    ...
    recentDonations: "SELECT SUM(amount) as total FROM donations WHERE date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)",
  };
  ...
  bcrypt.hash(defaultPassword, 10, (err, hashedPassword) => {
    ...
    db.query(query, [name, email, phone, hashedPassword, jobTitle], (err, result) => {
      if (err) {
        ...
        console.error("Database error in /api/staff POST:", err);
        return res.status(500).json({ message: "Database error", error: err.message });
      }
  ...
  db.query(query, [email, token], async (err, results) => {
    ...
    try {
      ...
      db.query(updateQuery, [hashedPassword, email], (err, result) => {
        if (err) {
          console.error("Database error in /api/reset-password/confirm (password update):", err.message);
          return res.status(500).json({ message: "Database error", error: err.message });
        }
        if (result.affectedRows === 0) {
        ...
        const deleteTokenQuery = "DELETE FROM password_reset_tokens WHERE email = ? AND token = ?";
        db.query(deleteTokenQuery, [email, token], (err) => {
          if (err) {
            console.error("Database error in /api/reset-password/confirm (token deletion):", err.message);
            // Note: Password was updated successfully, so we still return success
          }
